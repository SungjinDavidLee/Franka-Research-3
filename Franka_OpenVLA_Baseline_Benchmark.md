...
## 2. 단계별 평가 기준
  ...
  -------------------------------- ------------------- -------------------
                 ...
                 S2                목표 물체 접근      End-Effector가 목표
                                                       물체의 파지 가능한
                                                       접근 영역까지 도달

                 S3                파지 전 자세 정렬   목표 물체를 파지할
                                                       수 있도록
                                                       End-Effector의
                                                       위치와 자세를 정렬

                 S4                그리퍼 진입 및 접촉 목표 물체가 그리퍼
                                                       사이에 위치하고
                                                       유효한 접촉 발생

                 S5                안정적 파지         그리퍼 폐쇄 후 목표
                                                       물체의 파지 상태를
                                                       안정적으로 유지

                 S6                물체 들어 올리기    파지된 물체를
                                                       지지면에서 분리하여
                                                       안정적으로 상승
                 ...
                 S9                접시 위 정렬 및     물체를 접시의 유효
                                   하강                영역에 정렬한 후
                                                       안정적으로 하강

                S10                그리퍼 해제 및 최종 그리퍼 해제 후
                                   배치                물체가 접시 위에
                                                       안정적으로 배치
  ------------------------------------------------------------------------
